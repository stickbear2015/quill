...
| --- | --- | --- | --- | --- | --- |
| Tier 1 | Protect users and unlock the team | 20 | 20 | 0 | (complete) |
...
Tier 1 is complete: all seven bugs, the cheap security hardening, and the full gate ladder (GATE-1 through GATE-9) are committed and enforced in three CI workflows (`pr-ci.yml`, `security-ci.yml`, `accessibility-ci.yml`). The working tree is gate-clean: `ruff format --check .` passes, `ruff check . --ignore E501` passes, `mypy quill\core quill\io` reports zero errors, and the full suite is 701 passed / 7 skipped with 72.28% core+io coverage.
